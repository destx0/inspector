...
| `Shift + Enter` | In the palette: navigate to the highlighted checkpoint's route without restoring its state (`Shift + click` a row does the same) |
...
The palette can also navigate without touching the NgRx state. `Shift+Enter` (or `Shift+click`) on a checkpoint opens its saved route and leaves the current state exactly as it is. Typing a route directly into the search box shows a `Go to …` result: absolute paths like `/summary` are used as-is, while relative paths treat the current route as a directory — `./detail` appends to it (from `/workflow` → `/workflow/detail`) and `../next-page` moves up one level (from `/workflow/step1` → `/workflow/next-page`). When no checkpoint matches such a query, `Enter` simply navigates.
